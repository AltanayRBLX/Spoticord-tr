    ...
    if (err) {
      if (err.code === "ECONNREFUSED") {
        if (err.address === "127.0.0.1" && err.port === 4381) {
             ...
             **/
            log.error(" seems to be closed or unreachable on port 4381! Close Spotify and wait 10 seconds before restarting for this to work. Checking every 5 seconds to check if you've done so.");
...
songEmitter.on('songUpdate', song => {
  ...
  log(`Muzik guncellendi (oynatiliyor: ${song.playing})`)
});

rpc.on('ready', () => {
    log(`Discord'a baglanildi. (${appClient})`);
    global.intloop = setInterval(checkSpotify, 1500);
});
